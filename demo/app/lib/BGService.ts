...
const SETTINGS = {
  COMMON: [
    ...
    {name: 'distanceFilter', group: 'geolocation', dataType: 'integer', inputType: 'select', values: [0, 10, 50, 100, 1000], defaultValue: 10 },
...
export default class BGService {
  ...
  constructor() {
    this.uuid = null;
    this.state = {};
    this.os = Platform.device.os.toUpperCase();
    this.loadingIndicator = new LoadingIndicator();

    Settings.setBoolean('watchPosition', false);
    Settings.setNumber('watchPositionInterval', 1000);
    
    let items = [].concat(SETTINGS.COMMON).concat(SETTINGS[this.os]);
    this.settings = {
      items: items,
      map: {}
    };
  ...
  public playSound(key:string) {
    let soundId = SOUND_MAP[this.os][key.toUpperCase()]
    if (!soundId) {
  ...
  public getPlatformSettings(group?:string) {
    if (!group) {
  ...
  public setWatchPosition(enabled:boolean, interval:number) {
    Settings.setBoolean('watchPosition', enabled);
    Settings.setNumber('watchPositionInterval', interval);
  }
  /**
  * Returns an array of locations for iOS Simulator City Drive Route
  * @return {Array}
  */
  public getCityDriveData() {
