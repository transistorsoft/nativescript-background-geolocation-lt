...
const SETTINGS = {
  COMMON: [
    ...
    {name: 'debug', group: 'debug', dataType: 'boolean', inputType: 'select', values: [true, false], defaultValue: true},
    ...
    {name: 'geofenceProximityRadius', group: 'geolocation', dataType: 'integer', inputType: 'select', values: [0, 250, 500, 1000, 2500, 5000, 10000], defaultValue: 1000}
  ],
  IOS: [
    ...
    {name: 'disableStopDetection', group: 'activity_recognition', dataType: 'boolean', inputType: 'select', values: [true, false], defaultValue: false}
  ],
  ANDROID: [
...
let eventHandler = new Observable();
...
export class SettingsViewModel extends Observable {
  ...
  private _loiteringDelay:string;
  ...
  public static getState(callback:Function):any {
    BackgroundGeolocation.getState((state) => {
      if (!Settings.hasKey('uuid')) {
        // Initial boot
        state.params = {
          device: {
        ...
        Settings.setString('uuid', Platform.device.uuid);
      }
      this._state = state;
      callback(state);
    });    
  ...
  public static on(event, callback) {
    ...
    eventHandler.addEventListener(event, callback);
  }

  public static confirm(message, callback) {
    dialogs.confirm(message).then(function (result) {
      if (result === true) {
        callback(result);
      }
    });
  ...
  */
  public static playSound(name) {
    ...
    if (typeof(name) === 'string') {
      soundId = SOUND_MAP[Platform.device.os.toUpperCase()][name];
    } else if (typeof(name) === 'number') {
      soundId = name;
    }
    if (!soundId) {
      this.notify('Invalid sound id provided to BGService#playSound' + name);
      return;
    }
  ...
  public static notify(message) {
    let toast = Toast.makeText(message);
    toast.show();
  }

  constructor() {
    super();
    SettingsViewModel.getState((state) => {
      this._config = state;
    });
  ...
  set radius(value:string) {
    this._radius = value;
    this.notifyPropertyChange("radius", value);
  }
  ...
  get loiteringDelay():string {
    return this._loiteringDelay;
  }
  set loiteringDelay(value:string) {
    this._loiteringDelay = value;
    this.notifyPropertyChange("loiteringDelay", value);
  }
  ...
  public onClickResetOdometer(event) {
    SettingsViewModel.confirm('Reset odometer?', () => {
      BackgroundGeolocation.resetOdometer((value) => {
        var eventData: EventData = {
          eventName: "resetodometer",
          object: value
        };
        eventHandler.notify(eventData);
      })
    });    
  }
  ...
  public onClickSync(event) {
    BackgroundGeolocation.getCount((count) => {
      SettingsViewModel.confirm('Sync ' + count + ' records?', () => {
        BackgroundGeolocation.sync((rs) => {
          let message = 'Sync success (' + count + ' ' + ((count>0) ? 'records' : 'record') + ')';
          SettingsViewModel.notify(message);
          SettingsViewModel.playSound('MESSAGE_SENT');
        }, (error) => {
          dialogs.alert('Sync error: ' + error);
        });
      });
    });
  }
  
  public onClickDestroyLocations(event) {
    BackgroundGeolocation.getCount((count) => {
      if (!count) {
        SettingsViewModel.notify('Database is empty');
        return;
      }
      SettingsViewModel.confirm('Destroy ' + count + ' record(s)?', () => {
        BackgroundGeolocation.destroyLocations((rs) => {
          let message = 'Destroyed ' + count + ' record(s)';
          SettingsViewModel.notify(message);
        }, (error) => {
          dialogs.alert('Destroy locations error: ' + error);
        });
      });
    });
  }

  public onClickDestroyLog(event) {
    SettingsViewModel.confirm('Clear logs?', () => {
      BackgroundGeolocation.destroyLog(() => {
        SettingsViewModel.notify('Cleared logs');
      }, (error) => {
        dialogs.alert('Failed to clear log: ' + error);
      });
    });
  }

  public onClickEmailLog(event) {
    if (!this._email) {
  ...
  public onClickRemoveGeofences(event) {
    SettingsViewModel.confirm('Clear geofences?', () => {
      BackgroundGeolocation.removeGeofences([], () => {
        SettingsViewModel.notify('Cleared geofences');
        SettingsViewModel.playSound('MESSAGE_SENT');
      });
    });
  }

  public onClickLoadGeofences(event) {
    var data = CITY_DRIVE_DATA;
    var geofences = [];
    var radius = parseInt(this.radius);
    var loiteringDelay = parseInt(this.loiteringDelay);
    ...
    for (var n=0, len=data.length;n<len;n++) {
      geofences.push({
        ...
        extras: {
          radius: radius,
          center: {latitude: data[n].lat, longitude: data[n].lng}
        },
        latitude: data[n].lat,
    ...
    BackgroundGeolocation.addGeofences(geofences, () => {
      SettingsViewModel.playSound('ADD_GEOFENCE');
    }, (error) => {
